...
/**
 * Material system for the office aesthetic
 * Manages all materials used in the office environment with proper disposal
 */
export class OfficeMaterials {
  ...
  async initialize(renderer = null) {
    ...
    try {
      console.log("Initializing OfficeMaterials...");
      ...
      console.log("OfficeMaterials initialized successfully");
    } catch (error) {
  ...
  _initializeBasicMaterials() {
    ...
    // Ceiling material - clean white acoustic ceiling optimized for physically correct lighting
    const ceilingMaterial = new THREE.MeshStandardMaterial({
      color: 0xfcfcfc, // Very light warm white for realistic office ceiling
      metalness: 0.0, // Non-metallic material
      roughness: 0.8, // Slightly rough surface like acoustic tiles
      envMapIntensity: 0.15, // Very subtle environment reflections
    });
  ...
  getMaterial(name) {
    if (this.disposed) {
      throw new Error("OfficeMaterials has been disposed");
    }

    if (!this.initialized) {
      throw new Error(
        "OfficeMaterials not initialized. Call initialize() first"
      );
    }
