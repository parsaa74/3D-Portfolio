...
} from "../../../systems/corridorSystem.js";
import { CORRIDOR_MAP } from "../../../systems/map/CorridorMap.js";
...
export class OfficeEnvironment extends BaseEnvironment {
  constructor(options = {}) {
    super({
      ...options,
    ...
    this.materialSystem = new OfficeMaterials(this.renderer);
  ...
  async setupEnvironment() {
    console.log("Setting up office environment...");
  ...
  /**
   * Initialize the rendering materials with custom shaders for the office aesthetic
   * @private
   */
  async initializeMaterials() {
    ...
    // Initialize corridor lighting shader material - using clinical lighting
  ...
  /**
   * Creates a corridor network based on the corridor layout
   * @private
   */
  createCorridorNetwork() {
    ...
    console.log("Creating office corridor network from CORRIDOR_MAP...");
  ...
  createCorridorSegment(start, end, id) {
    ...
    // Add Twin Peaks-style strobing ceiling lights to corridor segments
    this._addCorridorLights(segment, effectiveLength, geometryCenterZ);
   ...
   */
    ...
    * @param {string} junctionId Identifier for the junction.
    * @returns {THREE.Group} The light fixture group with strobing capabilities.
    * @private
    */
   _createJunctionLightFixture(junctionSize, lightMaterial, junctionId) {
    const lightGroup = new THREE.Group();
    lightGroup.name = `junction_light_${junctionId}`;
    
    // Create the visual fixture
    ...
    const lightFixture = new THREE.Mesh(lightGeometry, fixtureMaterial);
    lightFixture.rotation.x = Math.PI / 2;
    lightFixture.position.y = CORRIDOR_HEIGHT - 0.05;
    lightGroup.add(lightFixture);
    
    // Create point light for actual illumination with localized range
    const pointLight = new THREE.PointLight(0xf7faff, this.strobeParams.baseIntensity * 1.5, 6.0, 2.0); // Reduced distance to 6.0 for localized effect, increased decay to 2.0
    pointLight.position.y = CORRIDOR_HEIGHT - 0.1;
    pointLight.userData = {
      isJunctionLight: true,
      baseIntensity: this.strobeParams.baseIntensity * 1.5,
      targetIntensity: this.strobeParams.baseIntensity * 1.5,
      flickerPhase: Math.random() * Math.PI * 2,
      fixture: lightFixture
    };
    lightGroup.add(pointLight);
    
    // Store light reference for strobing
    const lightId = `junction_${junctionId}_light`;
    this.strobeLights.set(lightId, pointLight);
    
    console.log(`Added strobing junction light ${lightId}`);
    return lightGroup;
   }
  ...
  /**
   * Adds Twin Peaks-style strobing ceiling lights to a corridor segment
   * @param {THREE.Group} segment The corridor segment group
   * @param {number} length The length of the corridor segment
   * @param {number} centerZ The Z position of the segment center
   * @private
   */
  _addCorridorLights(segment, length, centerZ) {
    const lightSpacing = 3.0; // Distance between lights
    const numLights = Math.max(1, Math.floor(length / lightSpacing));
    const lightGroup = new THREE.Group();
    lightGroup.name = `lights_${segment.name}`;
    
    // Create fluorescent tube material with emissive properties
    const tubeGeometry = new THREE.BoxGeometry(0.8, 0.05, 0.2);
    ...
    for (let i = 0; i < numLights; i++) {
      ...
      if (numLights === 1) {
        ...
        lightPosition.set(0, CORRIDOR_HEIGHT - 0.15, centerZ);
      } else {
        // Multiple lights evenly spaced
        const t = i / (numLights - 1);
        const zPos = centerZ - (length / 2) + (t * length);
        lightPosition.set(0, CORRIDOR_HEIGHT - 0.15, zPos);
      }

      // Create fluorescent tube fixture
      const tubeFixture = new THREE.Mesh(tubeGeometry, tubeMaterial);
      tubeFixture.position.copy(lightPosition);
      tubeFixture.rotation.x = Math.PI / 2;
      lightGroup.add(tubeFixture);

      // Create point light for actual illumination with localized range and influence
      const pointLight = new THREE.PointLight(0xf7faff, this.strobeParams.baseIntensity, 4.5, 2.0); // Reduced distance to 4.5 for tile-specific effect, increased decay to 2.0
      pointLight.position.copy(lightPosition);
      pointLight.userData = {
        isCorridorLight: true,
        baseIntensity: this.strobeParams.baseIntensity,
        targetIntensity: this.strobeParams.baseIntensity,
        flickerPhase: Math.random() * Math.PI * 2,
        fixture: tubeFixture
      };
      ...
      // Store light reference for strobing
      const lightId = `${segment.name}_light_${i}`;
      this.strobeLights.set(lightId, pointLight);
      
      console.log(`Added strobing light ${lightId} at position`, lightPosition);
    }

    segment.add(lightGroup);
    this.assets.lights.get("fluorescents").add(lightGroup);
  }
  ...
  createCorridorJunction(position, id) {
    ...
    const lightFixture = this._createJunctionLightFixture(junctionSize, lightMaterial, id);
  ...
  */ // KEEPING THE FUNCTION SIGNATURE AS A REMINDER TO ADD PORTFOLIO SETUP LATER
  ...
  /**
   * Set up the lighting system to match clinical aesthetic
   * @private
   */
  async setupLighting() {
  ...
  _setupCorridorLighting() {
    ...
    const lightMaterial = new THREE.MeshStandardMaterial({
      color: 0xffffff,
      emissive: 0xf7faff, // Slightly bluish tint for the clinical look
    ...
    // Initialize lighting system - no strobing for optimal performance
    this.strobeLights = new Map();
    this.strobeParams = {
      enabled: false, // Disabled for optimal performance
      baseIntensity: 3.5, // Steady lighting only
      maxIntensity: 3.5, // No variation
      flickerSpeed: 0, // No flicker
      strobeChance: 0, // No random strobes
      strobeCountdown: 0,
      lastStrobeTime: 0,
      moodiness: 0, // No atmospheric effects
      autoStrobeInterval: 999999, // Effectively disabled
      nextAutoStrobe: 0
    };

    // Add enhanced lighting for better poster visibility and overall brightness
    const directionalLight = new THREE.DirectionalLight(0xffffff, 0.6); // Increased from 0.2 to 0.6
    directionalLight.position.set(0, 1, 0);
    directionalLight.castShadow = false;
    this.scene.add(directionalLight);
    
    // Add ambient light to brighten up art rooms and posters
    const ambientLight = new THREE.AmbientLight(0xf7faff, 0.4); // Soft bluish ambient light
    this.scene.add(ambientLight);
    
    console.log("[Environment] Twin Peaks-style corridor lighting set up");
  }
  ...
  /**
   * Update lighting effects - Minimal strobing for performance
   * @private
   */
  updateLighting(deltaTime) {
    if (!this.strobeParams || !this.strobeLights) return;

    const time = performance.now() * 0.001; // Convert to seconds
    
    // Minimal strobing system - only affects individual lights, not all at once
    if (this.strobeParams.enabled) {
      // Process each light individually with its own timing
      for (const [lightId, light] of this.strobeLights) {
        if (!light || !light.userData) continue;

        const userData = light.userData;
        const baseIntensity = userData.baseIntensity || this.strobeParams.baseIntensity;
        
        let targetIntensity = baseIntensity;
        let emissiveIntensity = 1.0;

        // Initialize individual light timing if not set
        if (!userData.nextStrobeTime) {
          userData.nextStrobeTime = time + Math.random() * this.strobeParams.autoStrobeInterval;
          userData.strobeCountdown = 0;
        }

        // Check if this specific light should strobe
        const shouldStrobe = time >= userData.nextStrobeTime || 
          (Math.random() < this.strobeParams.strobeChance * deltaTime * 10); // Reduced frequency multiplier from 60 to 10

        if (shouldStrobe && userData.strobeCountdown <= 0) {
          // Start strobe for this individual light only
          userData.strobeCountdown = 0.1 + Math.random() * 0.3; // Shorter strobe duration (0.1-0.4 seconds)
          userData.nextStrobeTime = time + this.strobeParams.autoStrobeInterval + Math.random() * 10000; // Next strobe in 20-30 seconds
        }

        // Apply strobe effect if this light is currently strobing
        if (userData.strobeCountdown > 0) {
          // During strobe: moderate increase for this light only
          const strobeIntensity = this.strobeParams.maxIntensity * (0.8 + Math.random() * 0.4); // Less dramatic variation
          targetIntensity = strobeIntensity;
          emissiveIntensity = 2.0 + Math.random() * 1.0;
          
          // Occasional flicker during strobe (less frequent)
          if (Math.random() < 0.15) {
            targetIntensity = 0.2;
            emissiveIntensity = 0.2;
          }
          
          // Countdown this light's strobe timer
          userData.strobeCountdown -= deltaTime;
        } else {
          // Normal subtle flickering: minimal variations
          const flickerPhase = userData.flickerPhase + time * this.strobeParams.flickerSpeed;
          const flicker = Math.sin(flickerPhase) * 0.08 + Math.sin(flickerPhase * 1.7) * 0.05; // Reduced flicker intensity
          targetIntensity = baseIntensity + flicker;
          emissiveIntensity = 1.0 + flicker * 0.2;
          
          // Very occasional random dim (much less frequent)
          if (Math.random() < 0.005 * deltaTime * 10) {
            targetIntensity *= 0.7 + Math.random() * 0.2;
            emissiveIntensity *= 0.8;
          }
        }

        // Smooth intensity transitions (slower for less computational cost)
        const lerpSpeed = userData.strobeCountdown > 0 ? 8.0 : 1.5; // Reduced lerp speeds
        light.intensity = THREE.MathUtils.lerp(light.intensity, targetIntensity, deltaTime * lerpSpeed);
        
        // Update fixture emissive intensity (less frequently)
        if (userData.fixture && userData.fixture.material && Math.random() < 0.3) { // Only update 30% of the time
          userData.fixture.material.emissiveIntensity = THREE.MathUtils.lerp(
            userData.fixture.material.emissiveIntensity,
            emissiveIntensity,
            deltaTime * lerpSpeed * 0.5
          );
        }

        // Update phase for next frame (slower updates)
        userData.flickerPhase += deltaTime * this.strobeParams.flickerSpeed * 0.7;
      }
    }

    // Legacy emergency lighting support
    if (this.emergencyLighting) {
      this.lightFlickerIntensity = Math.random() * 0.4 + 0.6;
      const legacyLights = this.assets.lights.get("fluorescents");
      if (legacyLights) {
        legacyLights.children.forEach((lightGroup) => {
          lightGroup.children.forEach((child) => {
            if (child.isLight && !child.userData.isCorridorLight && !child.userData.isJunctionLight) {
              child.intensity = this.lightFlickerIntensity;
            }
          });
        });
      }
  ...
  /**
   * Toggle strobing lights on/off for performance
   * @param {boolean} enabled Whether to enable strobing effects
   */
  setStrobingEnabled(enabled) {
    if (this.strobeParams) {
      this.strobeParams.enabled = enabled;
      console.log(`Strobing lights ${enabled ? "enabled" : "disabled"}`);
      
      // If disabling, reset all lights to base intensity
      if (!enabled && this.strobeLights) {
        for (const [lightId, light] of this.strobeLights) {
          if (light && light.userData) {
            const baseIntensity = light.userData.baseIntensity || this.strobeParams.baseIntensity;
            light.intensity = baseIntensity;
            
            // Reset fixture emissive intensity
            if (light.userData.fixture && light.userData.fixture.material) {
              light.userData.fixture.material.emissiveIntensity = 1.0;
            }
          }
        }
      }
  ...
  dispose() {
    ...
    console.log("Disposing OfficeEnvironment");
  ...
  async _createDevInterior(interiorGroup, center, size, doorPosition) {
    ...
    // Floor removed to match office aesthetic
    
    // Store reference to floor shader for animation updates
    this._codeRoomFloorShader = null;
    
    // --- Creative Project Nodes ---
    const createProjectNode = async (position, nodeType) => {
      const nodeGroup = new THREE.Group();
      nodeGroup.position.copy(position);
      
      switch (nodeType) {
        case 'biome':
          // Semantic Biome: Organic typographic ecosystem
          return this._createBiomeNode(nodeGroup, position);
        case 'toys':
          // Philosophical Toys: Interactive timeline representation
          return await this._createToysNode(nodeGroup, position);
        case 'personal':
          // Personal Site: Architectural/portfolio structure
          return this._createPersonalNode(nodeGroup, position);
        case 'german_art':
          // German Art Schools: Abstract academic representation
          return this._createGermanArtNode(nodeGroup, position);
        default:
          return nodeGroup;
      }
    };

    // Semantic Biome: Create organic, flowing text-like organisms
    this._createBiomeNode = (nodeGroup, position) => {
      const biomeColor = new THREE.Color(0x00ff88); // Bright bio-green
      const emissiveColor = new THREE.Color(0x00cc66);
      ...
      // Create flowing character particles that look like organisms
      const characters = ['A', 'G', 'C', 'T', 'X', 'Y', 'Z', '0', '1'];
      
      for (let i = 0; i < 16; i++) {
        // Create text sprite for each "organism"
        const canvas = document.createElement('canvas');
        canvas.width = 64;
        canvas.height = 64;
        const ctx = canvas.getContext('2d');
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(characters[i % characters.length], 32, 32);
        ...
        // Position in organic flowing pattern
        const t = i / 16;
        const angle = t * Math.PI * 2 * helixTurns;
        const radius = helixRadius * (0.5 + 0.5 * Math.sin(t * Math.PI * 4));
        ...
        // Add to animations for organic movement
        const spriteId = `biome_organism_${i}`;
        sprite.name = spriteId;
        
        this._animatedObjects = this._animatedObjects || new Map();
        this._animatedObjects.set(spriteId, {
          object: sprite,
          centerX: sprite.position.x,
          centerZ: sprite.position.z,
          radius: radius,
          speed: 0.3 + (Math.random() * 0.2),
          phase: i * (Math.PI * 2 / 16),
          verticalAmplitude: 0.4,
          verticalFrequency: 0.8 + Math.random() * 0.4,
          isOrganic: true
        });
      }
      ...
      const biomeLight = new THREE.PointLight(biomeColor, 2.0, 3.0, 2);
      nodeGroup.add(biomeLight);
      
      return nodeGroup;
    };

    // Philosophical Toys: Create filmstrip model representation
    this._createToysNode = async (nodeGroup, position) => {
      const toyColor = new THREE.Color(0xffd700); // Gold
      
      // Load the filmstrip GLB model
      ...
      try {
        ...
        // Scale the filmstrip appropriately for the node
        const scale = 0.5; // Adjust as needed
        filmstrip.scale.set(scale, scale, scale);
        
        // Position the filmstrip
        filmstrip.position.set(0, 0, 0);
        
        // Add some rotation for dynamic effect
        filmstrip.rotation.x = Math.PI / 8; // Slight tilt
        filmstrip.rotation.y = Math.PI / 6; // Slight Y rotation
        
        // Enhance materials for golden philosophical aesthetic
        filmstrip.traverse((child) => {
          if (child.isMesh && child.material) {
            // Create enhanced material with golden tint
            const originalMaterial = child.material;
            const enhancedMaterial = originalMaterial.clone();
            
            // Add golden tint and emissive glow
            enhancedMaterial.color = new THREE.Color(0xffd700);
            enhancedMaterial.emissive = new THREE.Color(0xffaa00);
        ...
        nodeGroup.add(filmstrip);
        
        // Add floating philosophical "frames" around the filmstrip
        const frameCount = 6;
        const frameRadius = 1.2;
        
        for (let i = 0; i < frameCount; i++) {
          // Create small floating frames
          const frameGeom = new THREE.PlaneGeometry(0.15, 0.1);
          const frameMat = new THREE.MeshStandardMaterial({
            color: new THREE.Color().setHSL((i / frameCount) * 0.3 + 0.1, 0.8, 0.7),
          ...
          frame.position.set(
            ...
            Math.sin(i * 0.8) * 0.4, // Vertical variation
            Math.sin(angle) * frameRadius
          );
          
          // Make frames face the center
          frame.lookAt(0, frame.position.y, 0);
          
          nodeGroup.add(frame);
          
          // Add orbital animation for the frames
          const frameId = `philosophical_frame_${i}`;
          frame.name = frameId;
          
          this._animatedObjects = this._animatedObjects || new Map();
          this._animatedObjects.set(frameId, {
            object: frame,
            centerX: frame.position.x,
            centerZ: frame.position.z,
            radius: frameRadius,
            speed: 0.3 + (i * 0.02), // Slightly staggered speeds
            phase: i * (Math.PI * 2 / frameCount),
            verticalAmplitude: 0.3,
            verticalFrequency: 0.8,
            isPhilosophical: true
          });
        }
        
        // Add rotation animation to the main filmstrip
        const filmstripId = `philosophical_filmstrip`;
        filmstrip.name = filmstripId;
        
        this._animatedObjects = this._animatedObjects || new Map();
        this._animatedObjects.set(filmstripId, {
          object: filmstrip,
          centerX: 0,
          centerZ: 0,
          radius: 0,
          speed: 0.1, // Slow contemplative rotation
          phase: 0,
        ...
        // Warm philosophical lighting
        const toyLight = new THREE.PointLight(toyColor, 2.0, 3.0, 2);
        toyLight.position.set(0, 1.0, 0);
        nodeGroup.add(toyLight);
        
      } catch (error) {
        console.error('Failed to load filmstrip.glb:', error);
        
        // Fallback: Create a simple placeholder if model fails to load
        const fallbackGeom = new THREE.BoxGeometry(0.8, 0.1, 0.3);
        ...
        const fallbackLight = new THREE.PointLight(toyColor, 1.5, 2.5, 2);
        nodeGroup.add(fallbackLight);
      }
      
      return nodeGroup;
    };

    // Personal Site: Clean architectural structure
    this._createPersonalNode = (nodeGroup, position) => {
      const personalColor = new THREE.Color(0x4a90e2); // Professional blue
      const emissiveColor = new THREE.Color(0x2a5fa2);
      
      // Central architectural pillar
      const pillarGeom = new THREE.CylinderGeometry(0.08, 0.12, 1.2, 8);
      ...
      // Portfolio "pages" floating around
      const pageWidth = 0.3;
      const pageHeight = 0.4;
      const numPages = 6;
      
      for (let i = 0; i < numPages; i++) {
        const pageGeom = new THREE.PlaneGeometry(pageWidth, pageHeight);
        const pageMat = new THREE.MeshStandardMaterial({
          color: new THREE.Color().setHSL(0.6 + (i * 0.02), 0.3, 0.8), // Subtle blue variations
          emissive: emissiveColor,
          emissiveIntensity: 0.2,
          transparent: true,
          opacity: 0.8,
        ...
        const angle = (i / numPages) * Math.PI * 2;
        const radius = 0.6;
        const height = (i - numPages/2) * 0.15;
        ...
        page.rotation.y = -angle; // Face inward
        
        nodeGroup.add(page);
        
        // Professional, steady rotation
        const pageId = `portfolio_page_${i}`;
        page.name = pageId;
        
        this._animatedObjects = this._animatedObjects || new Map();
        this._animatedObjects.set(pageId, {
          object: page,
          centerX: page.position.x,
          centerZ: page.position.z,
          radius: radius,
          speed: 0.3, // Consistent, professional speed
          phase: i * (Math.PI * 2 / numPages),
          verticalAmplitude: 0.1,
          verticalFrequency: 0.5,
          isProfessional: true
        });
      ...
      const personalLight = new THREE.PointLight(personalColor, 1.2, 2.0, 2);
      nodeGroup.add(personalLight);
      
      return nodeGroup;
    };

    // German Art Schools: Abstract academic representation
    this._createGermanArtNode = (nodeGroup, position) => {
      const artColor = new THREE.Color(0xff4444); // Academic red
      const emissiveColor = new THREE.Color(0xcc2222);
      
      // Central academic "book" or knowledge core
      const bookGeom = new THREE.BoxGeometry(0.3, 0.4, 0.05);
      ...
      const radiusRange = [0.5, 0.8, 1.1];
      
      for (let i = 0; i < numElements; i++) {
        const elementGeom = artShapes[i % artShapes.length];
        const elementMat = new THREE.MeshStandardMaterial({
          color: new THREE.Color().setHSL(0.0 + (i * 0.02), 0.7, 0.5), // Red variations
          emissive: emissiveColor,
          emissiveIntensity: 0.3,
          metalness: 0.6,
          roughness: 0.4
        ...
        const element = new THREE.Mesh(elementGeom, elementMat);
        
        const angle = (i / numElements) * Math.PI * 2;
        const radius = radiusRange[i % radiusRange.length];
        const height = Math.sin(i * 0.7) * 0.4;
        ...
        nodeGroup.add(element);
        
        // Academic, thoughtful movement
        const elementId = `art_element_${i}`;
        element.name = elementId;
        
        this._animatedObjects = this._animatedObjects || new Map();
        this._animatedObjects.set(elementId, {
          object: element,
          centerX: element.position.x,
          centerZ: element.position.z,
          radius: radius,
          speed: 0.2 + (Math.random() * 0.1), // Slow, contemplative
          phase: i * (Math.PI * 2 / numElements),
          verticalAmplitude: 0.3,
          verticalFrequency: 0.4 + Math.random() * 0.2,
          isAcademic: true
        });
      }
      
      const artLight = new THREE.PointLight(artColor, 1.0, 2.5, 2);
      nodeGroup.add(artLight);
    ...
    for (const node of nodes) {
      const nodeGroup = await createProjectNode(node.position, node.type);
      ...
      switch(node.type) {
        case 'biome':
          nodeInteractionMesh.userData.link = 'https://parsaa74.github.io/Semantic-Biome/';
          nodeInteractionMesh.userData.infoContent = `The Semantic Biome - A three-dimensional typographic ecosystem where autonomous character-based organisms coexist and interact. Each entity exists within a complex volumetric ecosystem, governed by algorithmic behaviors that simulate evolutionary patterns and emergent intelligence.\n\nThis digital meditation explores how meaning emerges from seemingly random interactions between typographic organisms.`;
          break;
        case 'toys':
          nodeInteractionMesh.userData.link = 'https://parsaa74.github.io/Philosophical-Toys/';
          nodeInteractionMesh.userData.infoContent = `Philosophical Toys (2023) - A digital exploration of 19th century pre-cinema inventions that achieved motion within still images.\n\nThis interactive timeline reframes these "philosophical toys" as valuable objects beyond their role as cinema precursors. Using WebGL, three.js, and p5.js, the project offers both historical overview and contemporary interpretation of early film technology.\n\nDeveloped alongside a master's thesis on stillness and motion, it serves as an entry point for early film enthusiasts while exploring the relationship between physical and digital representations.`;
          break;
        case 'personal':
          nodeInteractionMesh.userData.link = 'https://parsaa74.github.io/Personal-Site/';
          nodeInteractionMesh.userData.infoContent = `Personal Portfolio Site - A clean, architectural presentation of creative and technical work. This site showcases the intersection of design thinking, technical implementation, and artistic vision.\n\nExplore a curated collection of projects spanning creative coding, interactive design, and digital art.`;
          break;
        case 'german_art':
          nodeInteractionMesh.userData.link = 'http://parsaa74.github.io/german-art-schools';
          nodeInteractionMesh.userData.infoContent = `German Art Schools - An academic exploration of institutional creative education and its impact on contemporary design practice. This project examines the relationship between traditional art education and modern creative methodologies.\n\nA contemplative study on the evolution of artistic pedagogy in the digital age.`;
          break;
        default:
          nodeInteractionMesh.userData.link = 'https://parsaa74.github.io/Personal-Site/';
          nodeInteractionMesh.userData.infoContent = `This is a creative project node representing the intersection of code and creativity. It showcases how mathematical operations can transform into visual experiences.\n\nClick the link to explore more.`;
      }
...
//     // Create the terminal screen
  ...
  async _createFilmInterior(interiorGroup, center, size, doorPosition) {
    ...
    // const poster2Texture = await textureLoader.loadAsync('/assets/textures/posters/Office_Photo_0201.jpg');
    ...
    const createFramedPoster = (posterTexture, position, rotation) => {
      ...
      // Frame material - matte black, matching office aesthetic
      const frameMaterial = new THREE.MeshStandardMaterial({
        color: 0x111111,
        roughness: 0.8,
        metalness: 0.2
